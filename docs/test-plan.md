...
**Playwright で自動化した**（`frontend/e2e/` に計10件。`cd frontend && npm run test:e2e`）。
CI では実行していない（理由は5章）。
...
| **E2E を CI で実行すること** | E2E 自体は Playwright で自動化済み（`frontend/e2e/` に計10件）。ただし CI には載せない。E2E 専用スタック（PostgreSQL + LocalStack + backend）の起動を CI が毎回負担することになり、現在の2ジョブ構成に対して実行時間の増加が大きいため。**テストコードの型検査は `npm run build` 経由で CI が行っている**ので、型の壊れた E2E テストは CI で止まる |
