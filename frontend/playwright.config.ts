 ...
 *
 * <b>CIで実行する</b>(2026-08-30に追加)。
 *
 * 以前は「CI側でスタックの起動時間を丸ごと負担するため、実行時間が大きく増える」
 * という理由で載せていなかった。**その判断を変えた。**
 * テストがあることと守られていることは別で、PRのたびに回らないテストは
 * いずれ壊れたまま気づかれなくなる。実際、CIに載せる作業の中で
 * <b>リフレッシュが二重に走る実装の穴</b>が見つかっている
 * (src/api/client.ts の refreshGeneration)。
 *
 * E2E コード自体は tsconfig.e2e.json 経由で `npm run build` の型検査対象でもあり、
